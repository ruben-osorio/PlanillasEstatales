...
function fileCustomExpression($file, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="t_sigep" && $field=="image")
	{
		;
		return $value;
	}	
				if($table=="t_facturas110" && $field=="archivo_digital")
	{
		;
		return $value;
	}	
				if($table=="t_multas" && $field=="doc_img")
	{
		;
		return $value;
	}	
				if($table=="t_doc_file" && $field=="cert_nac_img")
	{
		;
		return $value;
	}	
				if($table=="t_doc_file" && $field=="ci_img")
	{
		;
		return $value;
	}	
				if($table=="t_doc_file" && $field=="libreta_mil_img")
	{
		;
		return $value;
	}	
				if($table=="t_doc_file" && $field=="segip_img")
	{
		;
		return $value;
	}	
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="t_facturas110" && $field=="monto_total") 
	{
		return 0;
	}
				if($table=="t_facturas110" && $field=="monto_13") 
	{
		return 0;
	}
				if($table=="t_facturas110" && $field=="lugar") 
	{
		return 'LA PAZ';
	}
				if($table=="GENERACIÓN DEL PERSONAL QUE PRESENTA FACTURAS PARA LA RETENCIÓN IMPOSITIVA" && $field=="monto_total") 
	{
		return 0;
	}
				if($table=="GENERACIÓN DEL PERSONAL QUE PRESENTA FACTURAS PARA LA RETENCIÓN IMPOSITIVA" && $field=="monto_13") 
	{
		return 0;
	}
				if($table=="GENERACIÓN DEL PERSONAL QUE PRESENTA FACTURAS PARA LA RETENCIÓN IMPOSITIVA" && $field=="lugar") 
	{
		return 'LA PAZ';
	}
	return "";
