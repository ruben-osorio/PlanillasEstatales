...
function GetTableURL($table = "")
{
	...
	if("cta_banc" == $table) 
		return "cta_banc";
	if("t_descuentos" == $table) 
		return "t_descuentos";
	if("t_ingreso" == $table) 
		return "t_ingreso";
	if("identificacion_puesto" == $table) 
		return "identificacion_puesto";
	if("old_cas" == $table) 
		return "old_cas";
	if("ABONOS BANCARIOS" == $table) 
		return "ABONOS_BANCARIOS";
	if("AFP-PREVISION" == $table) 
		return "AFP_PREVISION";
	if("AFP-FUTURO" == $table) 
		return "AFP_FUTURO";
	if("t_salario_minimo" == $table) 
		return "t_salario_minimo";
	if("t_asignaciones" == $table) 
		return "t_asignaciones";
	if("t_caja" == $table) 
		return "t_caja";
	if("permanente" == $table) 
		return "permanente";
	...
	if("soc_secu" == $table) 
		return "soc_secu";
	if("t_tributaria" == $table) 
		return "t_tributaria";
	if("PLANILLA TRIBUTARIA" == $table) 
		return "PLANILLA_TRIBUTARIA";
	if("ASIGNACIONES FAMILIARES" == $table) 
		return "ASIGNACIONES_FAMILIARES";
	if("t_documentos" == $table) 
		return "t_documentos";
	if("t_ufv" == $table) 
		return "t_ufv";
	if("t_planilla_c" == $table) 
		return "t_planilla_c";
	if("t_planilla_h" == $table) 
		return "t_planilla_h";
	if("t_mes_activo" == $table) 
		return "t_mes_activo";
	if("t_planilla_c_mayores" == $table) 
		return "t_planilla_c_mayores";
	if("PLANILLA RETENCIONES" == $table) 
		return "PLANILLA_RETENCIONES";
	if("LISTADO DE CUMPLEAÑEROS" == $table) 
		return "LISTADO_DE_CUMPLEA_EROS";
	if("PERSONAL CON INAMOVILIDAD (prenatal, natalidad, lactancia)" == $table) 
		return "PERSONAL_CON_INAMOVILIDAD__prenatal__natalidad__lactancia_";
	if("PERSONAL CON CERTIFICADOS DEL CAS" == $table) 
		return "PERSONAL_CON_CERTIFICADOS_DEL_CAS";
	if("t_sigep" == $table) 
		return "t_sigep";
	if("PERSONAL REGISTRADO EN SIGEP" == $table) 
		return "PERSONAL_REGISTRADO_EN_SIGEP";
	if("t_facturas110" == $table) 
		return "t_facturas110";
	if("GENERACIÓN DEL PERSONAL QUE PRESENTA FACTURAS PARA LA RETENCIÓN IMPOSITIVA" == $table) 
		return "GENERACI_N_DEL_PERSONAL_QUE_PRESENTA_FACTURAS_PARA_LA_RETENCI_N_IMPOSITIVA";
	if("t_multas" == $table) 
		return "t_multas";
	if("t_doc_file" == $table) 
		return "t_doc_file";
	if("t_tipo_documento" == $table) 
		return "t_tipo_documento";
	if("t_tipo_multa" == $table) 
		return "t_tipo_multa";
...
{	
	...
	if("baja" == $shortTName) 
		return "baja";
	if("baja_con" == $shortTName) 
		return "baja_con";
	if("cta_banc" == $shortTName) 
		return "cta_banc";
	if("t_descuentos" == $shortTName) 
		return "t_descuentos";
	if("t_ingreso" == $shortTName) 
		return "t_ingreso";
	if("identificacion_puesto" == $shortTName) 
		return "identificacion_puesto";
	if("old_cas" == $shortTName) 
		return "old_cas";
	if("ABONOS_BANCARIOS" == $shortTName) 
		return "ABONOS BANCARIOS";
	if("AFP_PREVISION" == $shortTName) 
		return "AFP-PREVISION";
	if("AFP_FUTURO" == $shortTName) 
		return "AFP-FUTURO";
	if("t_salario_minimo" == $shortTName) 
		return "t_salario_minimo";
	if("t_asignaciones" == $shortTName) 
		return "t_asignaciones";
	if("t_caja" == $shortTName) 
		return "t_caja";
	if("permanente" == $shortTName) 
		return "permanente";
	if("char_per" == $shortTName) 
		return "char_per";
	if("contrato" == $shortTName) 
		return "contrato";
	if("soc_secu" == $shortTName) 
		return "soc_secu";
	if("t_tributaria" == $shortTName) 
		return "t_tributaria";
	if("PLANILLA_TRIBUTARIA" == $shortTName) 
		return "PLANILLA TRIBUTARIA";
	if("ASIGNACIONES_FAMILIARES" == $shortTName) 
		return "ASIGNACIONES FAMILIARES";
	if("t_documentos" == $shortTName) 
		return "t_documentos";
	if("t_ufv" == $shortTName) 
		return "t_ufv";
	if("t_planilla_c" == $shortTName) 
		return "t_planilla_c";
	if("t_planilla_h" == $shortTName) 
		return "t_planilla_h";
	if("t_mes_activo" == $shortTName) 
		return "t_mes_activo";
	if("t_planilla_c_mayores" == $shortTName) 
		return "t_planilla_c_mayores";
	if("PLANILLA_RETENCIONES" == $shortTName) 
		return "PLANILLA RETENCIONES";
	if("LISTADO_DE_CUMPLEA_EROS" == $shortTName) 
		return "LISTADO DE CUMPLEAÑEROS";
	if("PERSONAL_CON_INAMOVILIDAD__prenatal__natalidad__lactancia_" == $shortTName) 
		return "PERSONAL CON INAMOVILIDAD (prenatal, natalidad, lactancia)";
	if("PERSONAL_CON_CERTIFICADOS_DEL_CAS" == $shortTName) 
		return "PERSONAL CON CERTIFICADOS DEL CAS";
	if("t_sigep" == $shortTName) 
		return "t_sigep";
	if("PERSONAL_REGISTRADO_EN_SIGEP" == $shortTName) 
		return "PERSONAL REGISTRADO EN SIGEP";
	if("t_facturas110" == $shortTName) 
		return "t_facturas110";
	if("GENERACI_N_DEL_PERSONAL_QUE_PRESENTA_FACTURAS_PARA_LA_RETENCI_N_IMPOSITIVA" == $shortTName) 
		return "GENERACIÓN DEL PERSONAL QUE PRESENTA FACTURAS PARA LA RETENCIÓN IMPOSITIVA";
	if("t_multas" == $shortTName) 
		return "t_multas";
	if("t_doc_file" == $shortTName) 
		return "t_doc_file";
	if("t_tipo_documento" == $shortTName) 
		return "t_tipo_documento";
	if("t_tipo_multa" == $shortTName) 
		return "t_tipo_multa";
}
