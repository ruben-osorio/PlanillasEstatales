...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("funcionario" == $this->xml_array['tables'][0])
			$isDB = true;
		if("baja" == $this->xml_array['tables'][0])
			$isDB = true;
		if("baja_con" == $this->xml_array['tables'][0])
			$isDB = true;
		if("cta_banc" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_descuentos" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_ingreso" == $this->xml_array['tables'][0])
			$isDB = true;
		if("identificacion_puesto" == $this->xml_array['tables'][0])
			$isDB = true;
		if("old_cas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_salario_minimo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_asignaciones" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_caja" == $this->xml_array['tables'][0])
			$isDB = true;
		if("permanente" == $this->xml_array['tables'][0])
			$isDB = true;
		if("char_per" == $this->xml_array['tables'][0])
			$isDB = true;
		if("contrato" == $this->xml_array['tables'][0])
			$isDB = true;
		if("soc_secu" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_tributaria" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_tributaria" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_asignaciones" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_documentos" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_ufv" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla_c" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla_h" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_mes_activo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla_c_mayores" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla_c" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_planilla_c" == $this->xml_array['tables'][0])
		...
		if("old_cas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_sigep" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_sigep" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_facturas110" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_facturas110" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_multas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_doc_file" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_tipo_documento" == $this->xml_array['tables'][0])
			$isDB = true;
		if("t_tipo_multa" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
