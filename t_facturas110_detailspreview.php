...
$page_layouts["t_facturas110_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["id_fac"]));
	...
	//	mes_num - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("mes_num", $data, $keylink);
			$row["mes_num_value"] = $value;
			$format = $pSet->getViewFormat("mes_num");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("mes_num")))
				$class = ' rnr-field-number';
			$row["mes_num_class"] = $class;
	//	monto_total - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("monto_total", $data, $keylink);
			$row["monto_total_value"] = $value;
			$format = $pSet->getViewFormat("monto_total");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("monto_total")))
				$class = ' rnr-field-number';
			$row["monto_total_class"] = $class;
	//	monto_13 - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("monto_13", $data, $keylink);
			$row["monto_13_value"] = $value;
			$format = $pSet->getViewFormat("monto_13");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("monto_13")))
				$class = ' rnr-field-number';
			$row["monto_13_class"] = $class;
	//	lugar - 
			$viewContainer->recId = $recordsCounter;
	...
	//	archivo_digital - Document Download
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("archivo_digital", $data, $keylink);
			$row["archivo_digital_value"] = $value;
			$format = $pSet->getViewFormat("archivo_digital");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("archivo_digital")))
				$class = ' rnr-field-number';
			$row["archivo_digital_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("t_facturas110", "detailspreview"));
