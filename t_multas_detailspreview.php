...
require_once("include/t_multas_variables.php");
...
$page_layouts["t_multas_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["id_multas"]));
	
	
	//	id_multas - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("id_multas", $data, $keylink);
			$row["id_multas_value"] = $value;
			$format = $pSet->getViewFormat("id_multas");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("id_multas")))
				$class = ' rnr-field-number';
			$row["id_multas_class"] = $class;
	//	id_func - 
			$viewContainer->recId = $recordsCounter;
	...
	//	tipo_multa - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("tipo_multa", $data, $keylink);
			$row["tipo_multa_value"] = $value;
			$format = $pSet->getViewFormat("tipo_multa");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("tipo_multa")))
				$class = ' rnr-field-number';
			$row["tipo_multa_class"] = $class;
	//	monto - Number
			$viewContainer->recId = $recordsCounter;
	...
	//	doc_img - Document Download
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("doc_img", $data, $keylink);
			$row["doc_img_value"] = $value;
			$format = $pSet->getViewFormat("doc_img");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("doc_img")))
				$class = ' rnr-field-number';
			$row["doc_img_class"] = $class;
	//	mes - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("t_multas", "detailspreview"));
